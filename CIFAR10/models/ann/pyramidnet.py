...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if residual_channel != shortcut_channel:
            padding = torch.autograd.Variable(torch.zeros((batch_size, residual_channel - shortcut_channel, featuremap_size[0], featuremap_size[1]), dtype=torch.float, device='cuda'))
...
class PyramidNet(nn.Module):
    def __init__(self, depth, alpha, num_classes=10, bottleneck=False):
